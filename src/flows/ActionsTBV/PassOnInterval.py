...
class PassOnInterval(Action):
    ...
    def on_input_received(self, action_input=None):
        ...
        if (
            not self.verify_day(now)
            or not self.verify_month(now)
            or not self.verify_date(now)
            or not self.verify_time(now)
            or not self.verify_weekday(now)
        ):
            # then
            return (None, "*")
